...
test('shop branding ownership, private preview, publication, metadata and draft isolation',async()=>{
 ...
 const data={name:'ร้านชบา',description:'เรื่องราวร้าน',logo_key:'alice/photo',cover_key:'alice/photo',seo_title:'ชบา ออนไลน์',seo_description:'เลือกชมสินค้าชบา',published:false};
 ...
 let r=await call('/preview/'+id);let html=await r.text();assert.match(html,/สินค้าร่าง/);assert.match(html,/ฉบับร่าง · เฉพาะตัวอย่าง/);assert.doesNotMatch(html,/href="\/go\//);assert.match(html,/noindex,nofollow/);assert.equal((await DB.prepare('SELECT COUNT(*) AS n FROM events').first()).n,0);
 ...
 await call('/api/shops/'+id,'PUT',{name:'ชื่อใหม่',published:false});const saved=await DB.prepare('SELECT * FROM shops WHERE id=?').bind(id).first();assert.equal(saved.logo_key,'alice/photo');assert.equal(saved.seo_title,'ชบา ออนไลน์');assert.equal((await call('/media/alice%2Fphoto','GET',null,null)).status,401);
 }finally{DB.close();}
});
