...
const env={DB,APP_ENV:'local',CHECKOUT_HOSTS:process.env.CHECKOUT_HOSTS||'marketplace.example.test',IMPORT_HOSTS:process.env.IMPORT_HOSTS||'',
 ASSETS:{async fetch(req){const p=new URL(req.url).pathname;if(!['/index.html','/app.js','/styles.css','/product-editor.js','/video.js','/storefront.js','/cover-position.js','/shop-share.js','/product-analytics.js','/account-usage.js','/bulk-products.js','/import-review.js'].includes(p))return new Response('Not found',{status:404});return new Response(await readFile(resolve(root,'public',p.slice(1))),{headers:{'content-type':mime[extname(p)]}});}},
