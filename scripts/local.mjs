...
const env={DB,APP_ENV:'local',CHECKOUT_HOSTS:process.env.CHECKOUT_HOSTS||'marketplace.example.test',IMPORT_HOSTS:process.env.IMPORT_HOSTS||'',
 ASSETS:{async fetch(req){const p=new URL(req.url).pathname;if(!['/index.html','/app.js','/styles.css','/product-editor.js'].includes(p))return new Response('Not found',{status:404});return new Response(await readFile(resolve(root,'public',p.slice(1))),{headers:{'content-type':mime[extname(p)]}});}},
 MEDIA:{async put(key,bytes,options){const path=resolve(dir,'media',key);await mkdir(resolve(path,'..'),{recursive:true});await writeFile(path,bytes);await writeFile(path+'.json',JSON.stringify(options));},async get(key){if(!/^[a-zA-Z0-9/-]+$/.test(key))return null;try{return {body:await readFile(resolve(dir,'media',key)),...(JSON.parse(await readFile(resolve(dir,'media',key+'.json'),'utf8')))};}catch{return null;}}}}
